...
// Wire: RS232_TX (GPIO14) → RS232_SLAVE_RX (GPIO1)
//       RS232_SLAVE_TX (GPIO2) → RS232_RX (GPIO15)
...
// ============================================================
//  TEST — RS232  (dual-UART master ↔ slave cross-port test)
//
//  Wiring required on PCB:
//    RS232_TX (GPIO14) ──► RS232_SLAVE_RX (GPIO 1)
//    RS232_SLAVE_TX (GPIO 2) ──► RS232_RX (GPIO15)
//
//  Serial2 (muxRS232) = MASTER  → sends Modbus FC03 request
//  Serial1 (9600 baud) = SLAVE  → receives request, replies with
//                                  a valid FC03 response frame
//  PASS = master received & CRC-verified the slave response
//  WARN = slave port received nothing / CRC mismatch
// ============================================================
void testRS232() {
  testRunning = true;
  ...
  // ── Master port: RS232 via MUX ───────────────────────────
  muxRS232(); // A0=HIGH, A1=LOW
  Serial2.end();
  delay(20);
  Serial2.begin(MODBUS_BAUD, MODBUS_CFG, RS232_RX, RS232_TX); // RX=15 TX=14
  delay(50);

  // ── Slave port: Serial1 reconfigured to Modbus baud ─────
  Serial1.end();
  delay(20);
  // RX=1 receives RS232_TX (GPIO14); TX=2 sends back to RS232_RX (GPIO15)
  ...
  // ── Build Modbus FC03 request (master → slave) ───────────
  ...
  // ── Build canned FC03 response (slave → master) ──────────
  // Format: [ID][FC][ByteCount][Reg0 Hi][Reg0 Lo][Reg1 Hi][Reg1 Lo][CRC Lo][CRC Hi]
  uint8_t slaveResp[9] = {
    (uint8_t)FR_SLAVE_ID,
    MODBUS_FC03,
    (uint8_t)(FR_REG_COUNT * 2), // 4 data bytes
    0x00, 0x01,  // R0 = 1
    0x00, 0x02,  // R1 = 2
    0, 0
  };
  ...
  while (millis() < deadline) {
    ...
    // Step 1 — Master sends Modbus FC03 request
    drain(Serial2);
    drain(Serial1);
    Serial2.write(req, 8);
    Serial2.flush();

    // Step 2 — Wait for all 8 bytes to arrive at slave RX
    // 9600 baud × 8 bytes × 1.04 ms/byte ≈ 8.3 ms; use 25 ms margin
    delay(25);

    // Step 3 — Slave reads the incoming request
    uint8_t rxBuf[16];
    size_t rxn = 0;
    uint32_t rxt = millis();
    ...
    if (rxn >= 8 && rxBuf[0] == (uint8_t)FR_SLAVE_ID &&
        rxBuf[1] == MODBUS_FC03) {
      // Step 4 — Slave sends back the canned FC03 response
      drain(Serial1);
      Serial1.write(slaveResp, 9);
      Serial1.flush();
      logFmt("Slave  TX: %s\n", hexStr(slaveResp, 9).c_str());

      // Step 5 — Master reads & validates the response
      uint8_t resp[128];
    ...
    // Pace between rounds, keep web server responsive
  ...
  if (passes > 0) {
    setResult(T_RS232, S_PASS,
              "Cross-port OK — " + String(passes) + "/" +
                  String(attempts) + " rounds passed");
  } else {
    setResult(T_RS232, S_WARN,
              "MUX OK · slave port got no valid reply (" +
                  String(attempts) + " rounds)");
  }

  // Restore Serial1 to GPRS baud rate
  Serial1.end();
  delay(20);
