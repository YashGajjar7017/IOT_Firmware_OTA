...
// ============================================================
//  TEST — RS232  (MUX: A0=HIGH, A1=LOW  →  same bus logic as RS485)
//  Sends Modbus FC03 every 500 ms for 5 s.
//  PASS = at least one valid response received.
//  WARN = MUX switched OK but no device replied.
// ============================================================
void testRS232() {
  testRunning = true;
  setResult(T_RS232, S_PENDING, "Running…");
  logLine();
  logLn("RS232 · MODBUS RTU BUS TEST (5 s continuous)");
  ...
  // Modbus FC03 — read FR_REG_COUNT holding registers from FR_SLAVE_ID
  ...
  while (millis() < deadline) {
    ...
    // Pace sends — keep web server alive during wait
